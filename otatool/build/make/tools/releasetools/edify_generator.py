...
class EdifyGenerator(object):
  ...
  def __init__(self, version, info, fstab=None):
    ...
    if fstab is None:
      ...
      self.fstab = fstab
    self.fstab_com = self.info.get("fstab_com", None)
  ...
  def AssertDevice(self, device):
    ...
    if self.fstab_com:
      if device.endswith('u'):
        device_com = device[:-1]
      else:
        device_com = device + 'u'
      cmd = ('LogicalOr(getprop("ro.product.device") == "%s", getprop("ro.product.device") == "%s") || '
             'abort("E%d: This package is for \\"%s\\" devices; '
             'this is a \\"" + getprop("ro.product.device") + "\\".");') % (
                 device, device_com, common.ErrorCode.DEVICE_MISMATCH, device)
    else:
  ...
  def WriteRawImage(self, mount_point, fn, mapfn=None):
    ...
    fstab = self.fstab
    fstab_com = self.fstab_com
    if fstab:
      p = fstab[mount_point]
      if fstab_com:
        p_com = fstab_com[mount_point]
      ...
      if partition_type == "EMMC":
        if mapfn:
          ...
          split_fn = fn.split("/")
          if len(split_fn) >1 and fstab_com:
            if common.OPTIONS.ufs:
              compatible_dev = [p_com.device, p.device]
            else:
              compatible_dev = [p.device, p_com.device]
            str_script = "package_extract_file_compatible("
            if mount_point == "/spl":
              for cnt in range(len(split_fn) -1):
                str_script += '"'+ split_fn[cnt]+'"'+ ", " +'"'+ compatible_dev[cnt]+'"'+","
              str_script += '"' + split_fn[len(split_fn)-1]+'"'+ ", " + '"' +compatible_dev[len(split_fn)-1] + '"'
              str_script += " );"
              self.script.append(str_script)
            if mount_point == "/spl_bk":
              for cnt in range(len(split_fn) -1):
                str_script += '"'+ split_fn[cnt]+'"'+ ", " +'"'+ compatible_dev[cnt]+'"'+","
              str_script += '"' + split_fn[len(split_fn)-1]+'"'+ ", " + '"' +compatible_dev[len(split_fn)-1] + '"'
              str_script += " );"
    ...
    if input_path is None:
      if isinstance(input_zip, zipfile.ZipFile):
        ...
        if os.path.isdir(input_zip):
          data = open(os.path.join(input_zip,"OTA/bin/updater")).read()
    else:
