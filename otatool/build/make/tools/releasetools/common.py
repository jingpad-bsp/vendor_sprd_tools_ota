...
class Options(object):
  def __init__(self):
    ...
    self.default_path = "prebuilts/linux-x86/ota"
    self.search_path = platform_search_path.get(sys.platform)
    self.signapk_path = "framework/signapk.jar"  # Relative to search_path
    self.signapk_shared_library_path = "lib64"   # Relative to search_path
    self.unpac_perl_tool_path = "prebuilts/linux-x86/ota/pac_tools/unpac_perl"
...
def LoadInfoDict(input_file, repacking=False):
  ...
  if d.get("no_recovery") != "true":
    ...
    if OPTIONS.board_compatible:
      fstab = "OTA/recovery.emmc.f2fs.fstab"
      fstab_com = "OTA/recovery.ufs.f2fs.fstab"
      device = []
      if isinstance(input_file, zipfile.ZipFile):
        if fstab in input_file.namelist():
          device = input_file.read(fstab).split("\n")[0].split()[0]
      elif os.path.exists(input_file):
        device = open(os.path.join(input_file,fstab)).readline().split()[0]
      else:
        raise ValueError("recovery fstab not exits")
      if device == d["fstab"]["/spl"].device:
        d["ufs"] = False
        d["fstab_com"] = LoadRecoveryFSTab(read_helper, d["fstab_version"], fstab_com,system_root_image)
      else:
        d["ufs"] = True
        d["fstab_com"]  = LoadRecoveryFSTab(read_helper, d["fstab_version"], fstab,system_root_image)
...
def GetSparseImage(which, tmpdir, input_zip, allow_shared_blocks,
  ...
  for entry in image.file_map:
    ...
    if isinstance(input_zip, zipfile.ZipFile):
      ...
      if os.path.isdir(input_zip):
        if arcname not in os.listdir(input_zip):
          continue
    ...
    if RoundUpTo4K(info.file_size) > ranges.size() * 4096:
      print("www arcname: %s, has been set imcomplete"%(arcname))
...
def SignFile(input_name, output_name, key, password, min_api_level=None,
  ...
  java_library_path = os.path.join(
      OPTIONS.default_path, OPTIONS.signapk_shared_library_path)

  cmd = ([OPTIONS.java_path] + OPTIONS.java_args +
         ["-Djava.library.path=" + java_library_path,
          "-jar", os.path.join(OPTIONS.default_path, OPTIONS.signapk_path)] +
         extra_signapk_args)
  if whole_file:
    cmd.append("-w")
...
def GetUnpacTool(info_dict=None):
  """Get the unpac tool."""

  if OPTIONS.default_path:
    return os.path.join(os.path.abspath("."), OPTIONS.unpac_perl_tool_path, "unpac.pl")
...
def ParseOptions(argv,
  ...
  if OPTIONS.default_path:
    os.environ["PATH"] = (os.path.join(OPTIONS.default_path, "bin") +
