...
Non-A/B OTA specific options
  ...
  --repart
        Partitions changed between AndroidP and AndroidQ
...
def HasRecoveryPatch(target_files_zip):
  if isinstance(target_files_zip, zipfile.ZipFile):
    namelist = [name for name in target_files_zip.namelist()]
    return ("SYSTEM/recovery-from-boot.p" in namelist or
            "SYSTEM/etc/recovery.img" in namelist)
  else:
    if os.path.isdir(target_files_zip):
      namelist = [name for name in os.listdir(os.path.join(target_files_zip, "SYSTEM"))]
      return ("recovery-from-boot.p" in namelist or
               "recovery.img" in namelist)
...
def HasPartition(target_files_zip, partition):
  if isinstance(target_files_zip, zipfile.ZipFile):
    ...
    if os.path.isdir(target_files_zip):
      if partition.upper() in os.listdir(target_files_zip):
        return True
      else:
        return False
...
def HasSockoPartition(target_files_zip):
  return HasPartition(target_files_zip, "socko")

def HasOdmkoPartition(target_files_zip):
  return HasPartition(target_files_zip, "odmko")
...
def AddCompatibilityArchiveIfTrebleEnabled(target_zip, output_zip, target_info,
  ...
  def AddCompatibilityArchive(framework_updated, device_updated):
    ...
    for file_name in compatibility_files:
      if isinstance(target_zip, zipfile.ZipFile):
        ...
        if os.path.isdir(target_zip):
          ...
          if target_file_name in os.listdir(os.path.join(target_zip, "META")):
            data = open(os.path.join(target_zip, "META",target_file_name)).read()
            common.ZipWriteStr(compatibility_archive_zip, file_name, data)
...
""" % bcb_dev)
  ...
  if HasProductPartition(input_zip):
    system_progress -= 0.05
  if HasSockoPartition(input_zip) or HasOdmkoPartition(input_zip):
    system_progress -= 0.05
  ...
  def GetBlockDifference(partition):
    ...
    # do it.
    print("www partition %s processing"%(partition))
  ...
  if HasSockoPartition(input_zip):
    extra_block_diffs = [GetBlockDifference("socko")]
    progress_dict["socko"] = 0.05
  if HasOdmkoPartition(input_zip):
    extra_block_diffs.append(GetBlockDifference("odmko"))
    progress_dict["odmko"] = 0.05
  ...
  if target_info.get('use_dynamic_partitions') == "true":
    ...
    for block_diff in extra_block_diffs:
      block_diff.WriteScript(script, output_zip,
                             progress=progress_dict.get(block_diff.partition),
                             write_verify_script=OPTIONS.verify)
  else:
    for extra_block_diff in extra_block_diffs:
      block_diffs.append(extra_block_diff)
...
def WriteBlockIncrementalOTAPackage(target_zip, source_zip, output_file):
  ...
  if HasSockoPartition(target_zip):
    if not HasSockoPartition(source_zip):
      raise RuntimeError("can't generate incremental that adds /socko")
    socko_src = common.GetUserImage("socko", OPTIONS.source_tmp, source_zip,
                                     info_dict=source_info,
                                     allow_shared_blocks=allow_shared_blocks)
    hashtree_info_generator = verity_utils.CreateHashtreeInfoGenerator(
        "socko", 4096, target_info)
    socko_tgt = common.GetUserImage(
        "socko", OPTIONS.target_tmp, target_zip,
        info_dict=target_info,
        allow_shared_blocks=allow_shared_blocks,
        hashtree_info_generator=hashtree_info_generator)

    # Check first block of socko partition for remount R/W only if
    # disk type is ext4
    socko_partition = source_info["fstab"]["/socko"]
    check_first_block = socko_partition.fs_type == "ext4"
    disable_imgdiff = socko_partition.fs_type == "squashfs"
    socko_diff = common.BlockDifference("socko", socko_tgt, socko_src,
                                         check_first_block,
                                         version=blockimgdiff_version,
                                         disable_imgdiff=disable_imgdiff)
  else:
  ...
  if HasOdmkoPartition(target_zip):
    if not HasOdmkoPartition(source_zip):
      raise RuntimeError("can't generate incremental that adds /odmko")
    odmko_src = common.GetUserImage("odmko", OPTIONS.source_tmp, source_zip,
                                     info_dict=source_info,
                                     allow_shared_blocks=allow_shared_blocks)
    hashtree_info_generator = verity_utils.CreateHashtreeInfoGenerator(
        "odmko", 4096, target_info)
    odmko_tgt = common.GetUserImage(
        "odmko", OPTIONS.target_tmp, target_zip,
        info_dict=target_info,
        allow_shared_blocks=allow_shared_blocks,
        hashtree_info_generator=hashtree_info_generator)

    # Check first block of odmko partition for remount R/W only if
    # disk type is ext4
    odmko_partition = source_info["fstab"]["/odmko"]
    check_first_block = odmko_partition.fs_type == "ext4"
    disable_imgdiff = odmko_partition.fs_type == "squashfs"
    odmko_diff = common.BlockDifference("odmko", odmko_tgt, odmko_src,
                                         check_first_block,
                                         version=blockimgdiff_version,
                                         disable_imgdiff=disable_imgdiff)
  else:
...
""" % bcb_dev)
  ...
  if socko_diff:
    size.append(socko_diff.required_cache)
  if odmko_diff:
    size.append(odmko_diff.required_cache)
...
""" % bcb_dev)
  ...
  if socko_diff:
    socko_diff.WriteVerifyScript(script, touched_blocks_only=True)
  if odmko_diff:
    odmko_diff.WriteVerifyScript(script, touched_blocks_only=True)
  ...
  progress_dict = {"system": 0.6 if vendor_diff and product_diff and ( socko_diff or odmko_diff)else 0.9}
  if vendor_diff:
  ...
  if product_diff:
    ...
    progress_dict["product"] = 0.1
  if socko_diff:
    extra_block_diffs = [socko_diff]
    progress_dict["socko"] = 0.05
  if odmko_diff:
    extra_block_diffs.append(odmko_diff)
    progress_dict["odmko"] = 0.05
  if device_specific_diffs:
    block_diffs += device_specific_diffs
...
def main(argv):
  ...
  def option_handler(o, a):
    ...
    elif o == "--board_compatible":
      OPTIONS.board_compatible = True
    else:
      return False
    return True
  ...
  Green_color = "\033[1;36;40m"
  print(Green_color + "!!Begin to Gen OTA Pkg!!")
  ...
  if OPTIONS.info_dict.has_key("ufs"):
    OPTIONS.ufs = OPTIONS.info_dict["ufs"]
  ...
  if OPTIONS.incremental_source is not None:
    ...
    if OPTIONS.target_info_dict.has_key("ufs"):
      OPTIONS.ufs = OPTIONS.target_info_dict["ufs"]
    if os.path.isdir(OPTIONS.incremental_source):
      OPTIONS.source_info_dict = common.LoadInfoDict(OPTIONS.incremental_source)
    else:
  ...
  # Generate an incremental OTA.
    ...
    if os.path.isdir(OPTIONS.incremental_source):
      OPTIONS.source_tmp = OPTIONS.incremental_source
      source_zip = OPTIONS.incremental_source
    else:
      OPTIONS.source_tmp = common.UnzipTemp(OPTIONS.incremental_source, UNZIP_PATTERN)
      #with zipfile.ZipFile(OPTIONS.incremental_source, 'r') as source_zip:
    ...
    if isinstance(source_zip, zipfile.ZipFile):
  ...
  if isinstance(input_zip, zipfile.ZipFile):
    common.ZipClose(input_zip)

  Green_color = "\033[1;32;40m"
  print(Green_color + "!!Gen OTA Pkg DONE!!")
