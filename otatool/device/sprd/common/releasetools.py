...
def ReadFile(dir_input, filename):
  if isinstance(dir_input, zipfile.ZipFile):
    return dir_input.read(filename)
  else:
    if os.path.isdir(dir_input):
      return open(os.path.join(dir_input,filename)).read()
...
def ParseXml(input_zip, partition_list):
  ...
  xml_data = ReadFile(input_zip,"RADIO/partition.xml")
...
class EdifyGeneratorExt(object):
  ...
  def AddNvmergeToZipExt(self, input_zip, output_zip, input_path=None):
    ...
    if input_path is None:
      ...
      data_nv = ReadFile(input_zip,"OTA/bin/nvmerge")
      data_cfg = ReadFile(input_zip,"OTA/bin/nvmerge.cfg")
    else:
  ...
  def AddRepartsToZipExt(self, input_zip, output_zip, input_path=None):
    ...
    if input_path is None:
      ...
      data_repart = ReadFile(input_zip,"OTA/bin/repart")
    else:
  ...
  def AddE2fsToZipExt(self, input_zip, output_zip, input_path=None):
    ...
    if input_path is None:
      data_mkfs_f2fs = ReadFile(input_zip, "OTA/bin/make_f2fs")
      data_sload_f2fs = ReadFile(input_zip, "OTA/bin/sload_f2fs")
    else:
...
class PartitionFullUpdater(PartitionUpdater):
  ...
  def AddToOutputZip(self, output_zip, **kwargs):
    ...
    if (self.input.bin):
      ...
      if self.input.file_name not in output_zip.namelist():
...
def GetAllUpdaterConfigs(input_zip):
  ...
  try:
    ...
    config_file = ReadFile(input_zip,"OTA/bin/modem.cfg")
