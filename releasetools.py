...
OPTIONS.source_partition_list = []
OPTIONS.target_partition_list = []
OPTIONS.backup_list = ["prodnv", "miscdata", "recovery", "misc", "trustos", "trustos_bak", "sml", "sml_bak",
"uboot", "uboot_bak", "logo", "fbootlogo", "l_fixnv1", "l_fixnv2", "persist", "dtb", "dtbo", "vbmeta", "mvconfig", "secvm", "mobilevisor"]
...
def ParseXml(input_zip, partition_list):
  print"parse start!"
...
def ComparePartitionCompatible(options):
  print "IncrementalOTA_Install judge partition change or not"
  for i in range(0,len(options.target_partition_list)-1):
    src_part = options.source_partition_list[i]
    tgt_part = options.target_partition_list[i]
    if(src_part.name != tgt_part.name or src_part.size != tgt_part.size):
      print "************source and target package partition changed!!!***********"
      print "src_part name:%s, size=%s, pos=%s\n"%(src_part.name, src_part.size, src_part.pos)
      print "tgt part name:%s, size=%s, pos=%s\n"%(tgt_part.name, tgt_part.size, tgt_part.pos)
      print "*********************************************************************"
      print "*********************************************************************"
      print "*********************************************************************"
      print "****Warning ota doesn't support partition changed increment update***"
      print "*********************************************************************"
      print "*********************************************************************"
      sys.exit(1)
...
class EdifyGeneratorExt(object):
  ...
  def AddNvmergeToZipExt(self, input_zip, output_zip, input_path=None):
  ...
  def AddRepartsToZipExt(self, input_zip, output_zip, input_path=None):
    """Write the accumulated script to the output_zip file.  input_zip
    is used as the source for the 'updater' binary needed to run
    script.  If input_path is not None, it will be used as a local
    path for the binary instead of input_zip."""
  ...
  def AddE2fsToZipExt(self, input_zip, output_zip, input_path=None):
    """Write the accumulated script to the output_zip file.  input_zip
    is used as the source for the 'updater' binary needed to run
    script.  If input_path is not None, it will be used as a local
    path for the binary instead of input_zip."""
    ...
    if input_path is None:
      data_mkfs_f2fs = input_zip.read("OTA/bin/make_f2fs")
      data_sload_f2fs = input_zip.read("OTA/bin/sload_f2fs")
    else:
      data_mkfs_f2fs = open(os.path.join(input_path, "make_f2fs")).read()
      data_sload_f2fs = open(os.path.join(input_path, "sload_f2fs")).read()

    common.ZipWriteStr(output_zip, "META-INF/com/google/android/mkfs.f2fs",
                       data_mkfs_f2fs, perms=0755)
    common.ZipWriteStr(output_zip, "META-INF/com/google/android/sload.f2fs",
                       data_sload_f2fs, perms=0755)
...
class PartitionFile(object):
  ...
  def __init__(self, partition, file_name, input_dir, bootable=False, subdir=None):
    ...
    list_fn = file_name.strip().split("/")
    full_fn = [os.path.join(input_dir,fn) for fn in list_fn]
    for fn_cnt in range(len(list_fn)):
      if os.path.exists(full_fn[fn_cnt]):
        file_data = open(full_fn[fn_cnt]).read()
        self.bin.append(common.File(list_fn[fn_cnt], file_data))
        self.size += len(file_data)
      else:
        print ("[Warning:] no image file %s" % (full_fn[fn_cnt]))
        self.bin = None
        self.size = 0
...
class PartitionUpdater(object):
  ...
  def GetBuildProp(cls, key):
    if (cls.properties == None):
      cls.properties = cls.script.info.get("vendor.build.prop", {})
    try:
  ...
  def __init__(self, mount_point, file_name=None, target_ver_dir=None, source_ver_dir=None, extract=False, verbatim=False, mount_point2=None, mount_point3=None, nv_merge=None, spl_merge=False):
    ...
    if nv_merge or spl_merge:
      self.extract = True
      self.verbatim = True
      self.need_extract = OPTIONS.tmp_path
  ...
  def GetFixNvSize(self):
    ...
    if self.nv_merge == "" or self.nv_merge == "wcn":
      return "0x00"
    return PartitionUpdater.GetBuildProp("ro.vendor.modem.fixnv_size")
  ...
  def FullUpdateToPartition(self):
    ...
    file_name = self.target.file_name
    if extract:
    ...
    if not extract or self.need_extract is None:
      ...
      common.CheckSize(self.target.bin[0].data, mount_point_temp, self.options.info_dict)
    ...
    if self.nv_merge:
      ...
      new_nv = os.path.join(OPTIONS.tmp_path, self.target.file_name)
      ...
      if "fixnv" in self.partition.mount_point:
        if ("boca" not in self.target.file_name) and ("navajo" not in self.target.file_name):
          self.script_ext.Run_program(nvmerge_exe, nvmerge_cfg, self.GetRealDevicePath(p, pt_dev), self.GetRealDevicePath(p, pt_dev1),new_nv, self.GetFixNvSize())
        elif "boca" in self.target.file_name:
          self.script.AppendExtra("""if get_rftype() == "0" then """)
          self.script_ext.Run_program(nvmerge_exe, nvmerge_cfg, self.GetRealDevicePath(p, pt_dev), self.GetRealDevicePath(p, pt_dev1),new_nv, self.GetFixNvSize())
          self.script.AppendExtra("""endif; """)
        elif "navajo" in self.target.file_name:
          self.script.AppendExtra("""if get_rftype() == "1" then """)
          self.script_ext.Run_program(nvmerge_exe, nvmerge_cfg, self.GetRealDevicePath(p, pt_dev), self.GetRealDevicePath(p, pt_dev1),new_nv, self.GetFixNvSize())
          self.script.AppendExtra("""endif; """)
        self.script.AppendExtra(""" if last_run_status() == "1" then """)
        self.script.AppendExtra(""" abort("Failed to do nvmerge!!"); """)
        self.script.AppendExtra(""" endif; """)
...
class PartitionFullUpdater(PartitionUpdater):
  ...
  def AddToOutputZip(self, output_zip, **kwargs):
    ...
    list_file_name = self.input.file_name.split("/")
    for file_index in range(len(list_file_name)):
      if (self.input.bin):
        PartitionUpdater.AddToOutputZip(self, output_zip)
        if list_file_name[file_index] not in output_zip.namelist():
          common.ZipWriteStr(output_zip, list_file_name[file_index], self.input.bin[file_index].data)
        PartitionUpdater.total_update_size += self.input.size
      else:
        print ("no target %s; skipping." % self.file_name[file_index])
  ...
  def AddToOutputZip(self, output_zip, **kwargs):
    ...
    if self.update_flag:
      ...
      if self.verbatim:
        ...
        if self.target.file_name not in output_zip.namelist():
        ...
        if (self.file_name + ".p") not in output_zip.namelist():
...
def RepartPartition(script_ext, output_zip):
  print("\nRepartition userdata_fs_type : %s\n" % (OPTIONS.info_dict.get("userdata_fs_type"),))
  print("\nRepartition fs_type : %s\n" % (OPTIONS.info_dict.get("fs_type"),))
  script_ext.UnpackPackageFile("META-INF/com/google/android/reparts", os.path.join(OPTIONS.tmp_path, "repart"))
  script_ext.UnpackPackageFile("META-INF/com/google/android/mkfs.f2fs", os.path.join(OPTIONS.tmp_path, "mkfs.f2fs"))
  script_ext.UnpackPackageFile("META-INF/com/google/android/sload.f2fs", os.path.join(OPTIONS.tmp_path, "sload.f2fs"))
  repart_exe = os.path.join(OPTIONS.tmp_path, "repart")
  CreatePartitionCfgFile(output_zip, "partition.cfg")
  script_ext.UnpackPackageFile("partition.cfg", os.path.join(OPTIONS.tmp_path, "partition.cfg"))
  part_cfg = os.path.join(OPTIONS.tmp_path, "partition.cfg")

  userdata_fs_type = OPTIONS.info_dict.get("userdata_fs_type")
  if userdata_fs_type is not None:
    script_ext.Run_program(repart_exe, "-d", OPTIONS.device_path, "-c", part_cfg, "-b", OPTIONS.sdcard_path, "-k", "0", "-f", userdata_fs_type)
  else:
    userdata_fs_type = OPTIONS.info_dict.get("fs_type")
    if userdata_fs_type is not None:
      script_ext.Run_program(repart_exe, "-d", OPTIONS.device_path, "-c", part_cfg, "-b", OPTIONS.sdcard_path, "-k", "0", "-f", userdata_fs_type)
    else:
...
def AdjustPartition(info, input_zip, output_zip):
  ...
  script.AppendExtra(""" if is_mounted("%s") != "" then """%OPTIONS.sdcard_path)
  script.Print("repart gpt begin")
  RepartPartition(script_ext, output_zip)
  script.AppendExtra(""" if last_run_status() == "0" then """)
  script.AppendExtra("""abort("run program repart error!"); """)
  script.AppendExtra(""" else if last_run_status() == "3" then """)
...
def FullOTA_InstallBegin(info):
  ...
  script_ext.AddNvmergeToZipExt(input_zip, output_zip)
  ...
  if OPTIONS.partition_change:
    script_ext.AddRepartsToZipExt(input_zip, output_zip)
    script_ext.AddE2fsToZipExt(input_zip, output_zip)
    ParseXml(input_zip, OPTIONS.target_partition_list)
  ...
  #to write a empty odmkoimg into odmko partition since it will verify it on Q
  if OPTIONS.partition_change:
    odmko_prebuild_path = os.path.join(OPTIONS.input_tmp, "IMAGES/odmko_raw.img")
    odmko_img = common.File.FromLocalFile("odmko.img",odmko_prebuild_path)
    common.ZipWriteStr(output_zip, "odmko.img", odmko_img.data)
    script.Print("extract odmko img to /odmko ....")
    script.WriteRawImage("/odmko", "odmko.img")

    super_metadataonly_prebuild_path = os.path.join(OPTIONS.input_tmp, "IMAGES/super_mo.img")
    super_mo_img = common.File.FromLocalFile("super_mo.img",super_metadataonly_prebuild_path)
    common.ZipWriteStr(output_zip, "super_mo.img", super_mo_img.data)
    part_type, part_device = common.GetTypeAndDevice("/odmko", info.info_dict)
    super_dev = part_device[:-5] + "super"
    script.Print("extract super_metadata_only img to /super ....")
    script.AppendExtra('package_extract_file("super_mo.img", "%s");' % super_dev)
...
def IncrementalOTA_InstallBegin(info):
  ...
  script_ext.AddNvmergeToZipExt(target_zip, output_zip)
  ...
  print "IncrementalOTA_Install start ParseSourceXml"
  ParseXml(info.source_zip, OPTIONS.source_partition_list)
  print "IncrementalOTA_Install start ParseTargetXml"
  ParseXml(target_zip, OPTIONS.target_partition_list)
  ComparePartitionCompatible(OPTIONS)
...
def CreatePartitionCfgFile(output_zip, name):
  ...
  for p in OPTIONS.target_partition_list:
