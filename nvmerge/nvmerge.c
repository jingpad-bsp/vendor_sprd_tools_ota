...
static BOOLEAN checksum_64 = FALSE;
...
static unsigned short ___calc_checksum64(unsigned char *dat, unsigned long len)
{
    unsigned short num = 0;
    uint64_t chkSum = 0;
    while (len > 1) {
...
static BOOLEAN _chkEcc(uint8 * buf, uint32 size, uint16 checksum)
{
    ...
    crc = ___calc_checksum64(buf, size);
    NVMERGE_TRACE("NVMERGE:crc = 0x%x,checksum=0x%x\n", crc, checksum);
    if (crc == checksum) {
      checksum_64 = TRUE;
      NVMERGE_TRACE("NVMERGE:checksum_64=%d\n", checksum_64);
      return TRUE;
    }
...
static char *__checkNVPartition(char *ori_path, char *bak_path, uint32 * nv_len_ptr)
{
    ...
    nv_header_t *bak_header_ptr = NULL;
    ...
    do {
        ...
        if (!ori_header_ptr) {
            NVMERGE_TRACE("__checkNVPartition ori_header_ptr malloc failed\n");
            break;
        }
        ...
        if (!ori_buf) {
            NVMERGE_TRACE("__checkNVPartition ori_len malloc failed\n");
            break;
        }

        NVMERGE_TRACE("__checkNVPartition ori buf ecc\n");
        if (ori_len == read(ori_fd, ori_buf, ori_len)) {
            if (!_chkEcc(ori_buf, ori_len, ori_ecc)) {
                NVMERGE_TRACE("__checkNVPartition ori buf ecc error\n");
                break;
            }
            status += 1;
        }
    ...
    do {
        ...
        if (!bak_header_ptr) {
            NVMERGE_TRACE("__checkNVPartition bak_header_ptr malloc failed\n");
            break;
        }
        ...
        if (!bak_buf) {
            NVMERGE_TRACE("__checkNVPartition bak_buf malloc failed\n");
            break;
        }
    ...
    case 0:
        ...
        *nv_len_ptr = 0;
        if(ori_header_ptr && ori_buf && ori_len) {
            memset(ori_header_ptr, 0, SECTOR_SIZE);
            memset(ori_buf, 0, ori_len);
        } else {
            NVMERGE_TRACE("__checkNVPartition: ori_header_ptr, ori_buf or ori_len is null, fail to memset\n");
            break;
        }
        ...
        if(bak_fd < 0) {
            NVMERGE_TRACE("__checkNVPartition unable to fix bak partition, open bak_path failed\n");
            break;
        }
...
// check nv partition and fill in old_buf
    ...
    if(old_buf == NULL){
        NVMERGE_TRACE("NVMERGE:olb_buf is NULL\n");
        return FALSE;
    }
...
//out buffer
    ...
    if(NULL == out_buf){
        NVMERGE_TRACE("NVMERGE:malloc out_buf fail,malloc size id %d\n",fixnv_size + 4);
        free(old_buf);
        close(out_fd);
        return FALSE;
    }
    ...
    if(ret < 0){
        NVMERGE_TRACE("NVMERGE:read out_buf fail %d\n",fixnv_size + 4);
        free(old_buf);
        free(out_buf);
        close(out_fd);
        return FALSE;
...
//  write timestamp
    ...
    if(!merge_result){
        NVMERGE_TRACE("__mergeItem fail\n");
        free(out_buf);
        free(old_buf);
        return FALSE;
    }
    ...
    if(checksum_64)
      out_header_ptr->checksum = (uint32) ___calc_checksum64((unsigned char *)out_buf, fixnv_size);
    else
    ...
    if (ret == -1 || SECTOR_SIZE != write(out_fd, out_header_ptr, SECTOR_SIZE)
        || fixnv_size != write(out_fd, out_buf, fixnv_size)){
        NVMERGE_TRACE("NVMERGE:Write to output file fail\n");
        free(old_buf);
        free(out_buf);
        close(out_fd);
        return FALSE;
    ...
    out_fd = open(old_p1, O_RDWR | O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO);
    if (out_fd < 0) {
        NVMERGE_TRACE("NVMERGE:Open output file fail\n");
        free(old_buf);
        free(out_buf);
...
int main(int argc, char *argv[])
{
    ...
    fixnv_size = strtol(argv[5], 0, 16);
    NVMERGE_TRACE("NVMERGE:fixnv_size = 0x%x !\n", fixnv_size);
    if (!_merge(argv[2], argv[3], argv[4], argv[3])) {
